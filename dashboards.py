...
if senha == senha_correta:
    ...
    # 5 - Gerando graficos
    aba1, aba2 = st.tabs(['Dashboard', 'Extrair Dados'])
    ...
    with aba2:
        st.write("Aqui você pode extrair os dados do seu dashboard.")

        try:
            st.download_button(
                label="Baixar Base de Dados Geral",
                data=dt_df_filtered.to_json(),
        ...
        try:
            st.download_button(
                label="Baixar Base de Dados Ativos",
                data=dt_df_filtered_resultado.to_json(),
                file_name='dados_resultado.json',
                mime="application/json",
            )
        except: 
            colored('Erro ao baixar o arquivo')

        try:
            st.download_button(
                label="Baixar Base de Dados Devoluções",
                data=dt_df_filtered_devolutivo.to_json(),
                file_name='dados_devolutivo.json',
                mime="application/json",
            )
        except: 
            colored('Erro ao baixar o arquivo')
